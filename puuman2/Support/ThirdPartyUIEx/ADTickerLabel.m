...
// last edit: Declan 陈晔 添加对小数点等特殊字符的支持，添加自动居中。
...
- (void)setCharactersArray:(NSArray *)charactersArray{
    ...
    CGSize size = [string sizeWithFont:self.textLabel.font constrainedToSize:CGSizeMake(self.textLabel.frame.size.width, MAXFLOAT)];
...
- (void)insertNewTickerCharacterView:(NSString *)ch
{
    BOOL num = [ch isEqualToString:@"0"] || [ch integerValue] > 0;
    NSArray *charactersArray = num? self.charactersArray : @[ch];
    CGFloat width = num? self.characterWidth : self.notNumCharacterWidth;
    ...
    tickerCharacterViewFrame.size = newTextLabelBounds.size;
    tickerCharacterViewFrame.origin.y = (self.scrollDirection == ADTickerLabelScrollDirectionDown) ? -self.font.lineHeight * ([charactersArray count] - 1) : 0;
    ...
    if (num) numbericView.selectedCharacter = @"0";
    numbericView.selectedCharacterIndex = num ? [self.charactersArray count] - 1 : 1;
...
- (void)updateFrame{
    ...
    float originX = 0;
    for (ADTickerCharacterView *numericView in self.characterViewsArray)
    {
        ...
        originX += numericView.frame.size.width;
    }

    CGRect newViewFrame = self.frame;
    newViewFrame.size.width = originX;
...
- (void)setText:(NSString *)text{
    ...
    if(![_text isEqualToString: text]){
        ...
        for (int i=0; i<oldTextLength; i++) [self deleteCharacterView];
        for(int i = 0 ; i < newTextLength; i++)
        {
            NSString *ch = [text substringWithRange:NSMakeRange(i, 1)];
            [self insertNewTickerCharacterView:ch];
        }
        ...
        [self.characterViewsArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            ...
            ADTickerCharacterView *numericView = (ADTickerCharacterView *)obj;
            NSString *ch = [text substringWithRange:NSMakeRange(idx, 1)];
            [numericView setSelectedCharacter:ch animated:YES];
        }];
